 ...
 * Handles Firestore profile updates and Supabase Storage image uploads.
 */
@Singleton
class ProfileRepositoryImpl @Inject constructor(
    ...
    companion object {
        ...
        private const val COVER_BUCKET = "vendor-covers"
    }
    ...
    override suspend fun updateVendorProfile(
        ...
        return try {
            ...
            // Fetch and return updated vendor profile
    ...
    override suspend fun getVendorProfile(vendorId: String): AuthResult<VendorProfile> {
        return try {
            val snapshot = firestore.collection(VENDORS_COLLECTION)
                .document(vendorId)
                .get()
                .await()
    ...
    override fun observeVendorProfile(vendorId: String): Flow<VendorProfile?> = callbackFlow {
        val listener = firestore.collection(VENDORS_COLLECTION)
            .document(vendorId)
            .addSnapshotListener { snapshot, error ->
                if (error != null) {
                    close(error)
